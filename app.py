...
if st.session_state.current_page == "calculator":
    ...
    if campaign_name == "New Customers":
        ...
        st.sidebar.info("Select the metrics required for this payout:")
        use_coll_amt = st.sidebar.checkbox("Apply Min Collection Amount", value=True)
        if use_coll_amt:
            coll_amt_t = st.sidebar.number_input("Base Min Amount", value=50000.0)
...
if st.session_state.current_page == "admin":
    ...
    if perf_file:
        try:
            ...
            if campaign_name == "New Customers": 
                q = q[q['New_Customers'] >= (new_cust_t * mult)]
            elif campaign_name == "Unique Customers": 
                q = q[q['Unique_Customers'] >= (unique_cust_t * mult)]
            elif campaign_name == "Active Customers": 
                q = q[q['Active_Customers'] >= (active_cust_t * mult)]
            elif campaign_name == "Dormant Customers": 
                q = q[q['Dormant_Customers'] >= (dormant_cust_t * mult)]
                ...
                q = q[q['Disbursements'] >= disb_pct_t]
            ...
            # Dynamic Disbursement Logic: (Extra % / User-defined Step) * Step Reward
            ...
            # Final View
            cols = ['Sector', 'Subsector', 'Branch', 'Pair_ID', 'Staff_Name', 'Phone_Number', 'Role', 'Unit_Count', 'Target_Multiplier', 'Staff_Payout_Amount']
